...
export function resize(point0, point1, fromStart, debug) {
    ...
    var $resizerEl = $(eventEl).find('.' + (fromStart ? CalendarWrapper.EVENT_START_RESIZER_CLASSNAME : CalendarWrapper.EVENT_END_RESIZER_CLASSNAME)).css('display', 'block'); // usually only displays on hover. force display
    var resizerRect = $resizerEl[0].getBoundingClientRect();
    var resizerCenter = getRectCenter(resizerRect);
    var vector = subtractPoints(resizerCenter, point0);
    var endPoint = addPoints(point1, vector);
