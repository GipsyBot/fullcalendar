...
describe('events as a json feed', function () {
    ...
    it('requests correctly when local timezone', function (done) {
        var START = '2014-04-27T00:00:00';
        var END = '2014-06-08T00:00:00';
    ...
    it('processes new events under updated time zone', function (done) {
        XHRMock.get(/^my-feed\.php/, function (req, res) {
            var reqTimeZone = req.url().query.timeZone;
        ...
        setTimeout(function () {
            var eventStartStr = calendar.getEvents()[0].startStr;
    ...
    it('calls loading callback', function (done) {
        var loadingCallArgs = [];
        XHRMock.get(/^my-feed\.php/, function (req, res) { return res.status(200).header('content-type', 'application/json').body('[]'); });
        initCalendar({
