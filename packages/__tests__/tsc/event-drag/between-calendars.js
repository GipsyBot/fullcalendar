...
describe('dragging events between calendars', function () {
    ...
    it('fires all triggers', function (done) {
        ...
        calendar0.render();
        calendar1.render();
        var dayGridWrapper0 = new DayGridViewWrapper(calendar0).dayGrid;
        var dayGridWrapper1 = new DayGridViewWrapper(calendar1).dayGrid;
        eventEl = dayGridWrapper0.getEventEls()[0];
        var dayEl = dayGridWrapper1.getDayEls('2019-01-05')[0];
        var point1 = getRectCenter(dayEl.getBoundingClientRect());
    ...
    it('works between timeGrid views', function (done) {
        ...
        calendar0.render();
        calendar1.render();
        var eventEl = new CalendarWrapper(calendar0).getEventEls()[0]; // of the source calendar
        var destViewWrapper = new TimeGridViewWrapper(calendar1);
        var point1 = getRectCenter(destViewWrapper.getScrollerEl().getBoundingClientRect());
        $(eventEl).simulate('drag', {
            end: point1,
        });
    });
