import * as EventResizeUtils from '../lib/EventResizeUtils';
import { DayGridViewWrapper } from '../lib/wrappers/DayGridViewWrapper';
describe('validRange event resizing', function () {
    describe('when in month view', function () {
        pushOptions({
            initialView: 'dayGridMonth',
            initialDate: '2017-06-01',
            validRange: { end: '2017-06-09' },
            events: [
                { start: '2017-06-04', end: '2017-06-07' },
            ],
            editable: true,
        });
        it('won\'t go after validRange', function (done) {
            var calendar = initCalendar();
            var dayGridWrapper = new DayGridViewWrapper(calendar).dayGrid;
            EventResizeUtils.resize(dayGridWrapper.getDayEl('2017-06-06').getBoundingClientRect(), dayGridWrapper.getDisabledDayEls()[0].getBoundingClientRect()).then(function (res) {
                expect(res).toBe(false);
            }).then(done);
        });
    });
});
//# sourceMappingURL=validRange.js.map