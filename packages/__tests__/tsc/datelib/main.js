    ...
    describe('when UTC', function () {
        ...
        describe('range formatting', function () {
            ...
            it('works with different days of same month, with inprecise formatter', function () {
                var otherFormatter = createFormatter({
                    month: 'long',
                    year: 'numeric',
                });
                var m0 = env.createMarker('2018-06-08');
                var m1 = env.createMarker('2018-06-09');
                var s = env.formatRange(m0, m1, otherFormatter);
        ...
        describe('diffWholeYears', function () {
            it('returns null if not whole', function () {
                ...
                var diff = env.diffWholeYears(env.createMarker(d0), env.createMarker(d1));
        ...
        describe('diffWholeMonths', function () {
            it('returns null if not whole', function () {
                ...
                var diff = env.diffWholeMonths(env.createMarker(d0), env.createMarker(d1));
                expect(diff).toBe(null);
            });
            it('returns negative', function () {
        ...
        describe('diffWholeWeeks', function () {
            it('returns null if not whole', function () {
                ...
                var diff = diffWholeWeeks(env.createMarker(d0), env.createMarker(d1));
                expect(diff).toBe(null);
            });
            it('returns negative', function () {
        ...
        describe('diffWholeDays', function () {
            it('returns null if not whole', function () {
                ...
                var diff = diffWholeDays(env.createMarker(d0), env.createMarker(d1));
                expect(diff).toBe(null);
            });
            it('returns negative', function () {
                var d0 = new Date(Date.UTC(2018, 5, 19));
        ...
        describe('diffDayAndTime', function () {
            it('returns negative', function () {
                ...
                var d1 = new Date(Date.UTC(2018, 5, 5));
                var diff = diffDayAndTime(env.createMarker(d0), env.createMarker(d1));
                expect(diff).toEqual({
                    years: 0,
                    months: 0,
                    days: -14,
        ...
        describe('ISO8601 parsing', function () {
            ...
            it('does not lose info when parsing a dst-dead-zone date', function () {
                ...
                if (!deadZone) {
                    ...
                    var utcDate = new Date(Date.UTC(deadZone[1].getFullYear(), deadZone[1].getMonth(), deadZone[1].getDate(), deadZone[1].getHours() - 1, // back one hour. shouldn't exist in local time
                    deadZone[1].getMinutes(), deadZone[1].getSeconds(), deadZone[1].getMilliseconds()));
                    var s = formatIsoWithoutTz(utcDate);
                    // check that the local date falls out of the dead zone
                    var localDate = new Date(s);
